...

{
    ...
    struct Position preferedPos;
     int row=0;
    int clum=0;
    for(clum=0;clum<BOARD_SIZE;clum++)
    {
    	for(row=0;row<BOARD_SIZE;row++)
    	{
    		if(OTHER==board[row][clum]&&OTHER==board[row+1][clum]&&board[row+2][clum]&&EMPTY==board[row+3][clum])
    		{
    			preferedPos.x=row+3;
    			preferedPos.y=clum;
    			return preferedPos;
    		...
    		else if(OTHER==board[row][clum]&&OTHER==board[row][clum+1]
			&&OTHER==board[row][clum+2]&&EMPTY==board[row][clum+3])
			{
				preferedPos.x=row;
				preferedPos.y=clum+3;
				return preferedPos;
				goto there;
	        }
		}    
	}
    for (i = 9; i < BOARD_SIZE; i++)
        for (j = 9; j < BOARD_SIZE; j++)
        {
                for(int a=i;a<BOARD_SIZE;a++)
                {
                	if(EMPTY==board[a][j])
                	{
                		preferedPos.x=a;
